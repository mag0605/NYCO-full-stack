...
In this challenge, we ask you to build a simple API and write some JavaScript to request data from
the API and transform the response.

## Task 1 - Build Records API

In `/records/index.js`, complete API endpoint so that the API returns a JSON array of items in the following format:
...
- Don't add any additional libraries or edit any files other than `api/managed-records.js` and `/records/index.js`.
